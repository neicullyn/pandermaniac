...
if __name__ == '__main__':
    ...
    NUM_MUTATIONS = 8
    NUM_RANDOMS = 8
    NUM_STRATEGIES = NUM_DEFAULT + NUM_WINNING + NUM_MUTATIONS + NUM_RANDOMS
    ...
    default_strategies = []
    default_strategies_files = os.listdir(os.path.join('training', 'default_strategies'))
    ...
    # ************ modify so IT_NUM in range(MX_CUR, MX_ITE) ************
    ...
    for ITE_NUM in range(ITE_CUR, MAX_ITE):
        ...
        for stg_idx in range(NUM_MUTATIONS):
            ...
            stg_nodes_dict[mut_stg_idx] = mutate_group[stg_idx]

        # random_group = []
        for stg_idx in range(NUM_RANDOMS):
            rand_stg_idx = stg_idx + NUM_WINNING + NUM_RANDOMS
            s = strategy.Strtg()
            s.random_weight(strategy.preprocess({}))
            # random_group.append(s)
            stg_nodes_dict[rand_stg_idx] = s


        # for key, value in stg_nodes_dict.items():
            # print value
    ...
    # #     last_step_strategies = last_step_strategies.values()
